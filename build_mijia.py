...
def build(run, live=False, tests=False):
    ...
    with tempfile.TemporaryDirectory(prefix='mijia-', dir=ROOT / 'build') as directory:
        ...
        sources = sorted((ROOT / 'mijia/src').rglob('*.java'))
        if tests: sources.append(ROOT / 'tests/MijiaTest.java')
    ...
    print('完成：米家服务编译与 DEX 校验；独立服务对应源码随包提供。')
...
if __name__ == '__main__':
    ...
    build(run, '--live' in sys.argv, '--test' in sys.argv or '--live' in sys.argv)
