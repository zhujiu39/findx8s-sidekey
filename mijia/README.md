...
- 每次米家操作完成后重新批量读取快捷栏可显示的布尔属性；切换需要的操作前读取与操作后确认各执行一次，同一属性的重复绑定合并。空闲时不读取云端。开/关/未知/非开关分别用 `1`/`0`/`?`/`n` 表示，不把账号、设备编号或凭据送给菜单组件。
- `mijia-menu.log` 和 `mijia/debug.log` 记录本地提交、排队、发送、状态读回及耗时，每个日志超过 32 KiB 时轮换，最多保留一份旧文件。WebUI 只展示末尾的诊断内容；日志排除原始 HTTP 响应、凭据、完整设备编号和会话令牌。
