...
- 每次米家操作完成后重新批量读取快捷栏可显示的布尔属性；同一属性的重复绑定合并。布尔操作核对目标值，首次一致立即完成；不一致或返回未知时，以 250/500/1000/1500 ms 的间隔仅复查目标属性，最多补查 4 次，全部读取和等待共享 15 秒预算，可取消。不会重复发送控制。确认期间本地返回 `P`，达上限仍不一致时该属性的所有绑定显示未知，记录 `STATE_UNCONFIRMED`。场景没有明确目标值，操作后只刷新一轮设备状态。
...
- 空闲时不读取云端。开/关/未知/非开关分别用 `1`/`0`/`?`/`n` 表示，不把账号、设备编号或凭据送给菜单组件。
