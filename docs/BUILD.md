...
米家服务可以单独使用 `python build_mijia.py` 构建；显式添加 `--test` 才运行主机测试，`--live` 另行验证匿名扫码握手和二维码读取。详见 [米家服务说明](../mijia/README.md)。构建不会执行 Git push、创建 Tag 或发布 Release。
