...
int main(int argc, char **argv)
{
    ...
    FILE *control_output = tmpfile(); assert(control_output);
    assert(mijia_control_request(control_output, token, ids, 2, ids[0], ids[1]));
    rewind(control_output); char control_text[512] = {0};
    assert(fread(control_text, 1, sizeof(control_text) - 1, control_output) > 0);
    assert(strstr(control_text, "\"op\":\"menu-control\"") && strstr(control_text, "\"requestId\":\"abcdef"));
    fclose(control_output);
    assert(!mijia_control_request(stdout, token, ids, 0, ids[0], ids[1]));
    assert(!mijia_control_request(stdout, token, ids, 1, ids[1], ids[0]));
    assert(!mijia_control_request(stdout, token, ids, 2, ids[0], "bad"));
    FILE *result_output = tmpfile(); assert(result_output);
    assert(mijia_result_request(result_output, token, ids[1]));
    rewind(result_output); memset(control_text, 0, sizeof(control_text));
    assert(fread(control_text, 1, sizeof(control_text) - 1, result_output) > 0);
    assert(strstr(control_text, "\"op\":\"menu-result\"") && !strstr(control_text, "\"id\":"));
    fclose(result_output);
    char full_ids[MIJIA_BINDING_CAP][33];
    for (uint32_t i = 0; i < MIJIA_BINDING_CAP; i++) strcpy(full_ids[i], ids[0]);
    FILE *full_output = tmpfile(); assert(full_output);
    assert(mijia_control_request(full_output, token, full_ids, MIJIA_BINDING_CAP, ids[0], ids[1]));
    assert(ftell(full_output) < 9196); fclose(full_output);
    snprintf(text, sizeof(text), "PING %s", token);
