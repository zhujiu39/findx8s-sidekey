...
[![Version](https://img.shields.io/badge/version-v0.4.4%20%7C%20pre--release-orange)](https://github.com/zhujiu39/findx8s-sidekey/releases/tag/v0.4.4)
...
[下载模块](https://github.com/zhujiu39/findx8s-sidekey/releases/tag/v0.4.4) ·
[快速开始](#quick-start) ·
[支持的动作](#actions) ·
[常见问题](#faq) ·
[从源码构建](#build) ·
...
> [!TIP]
> 想直接安装？下载 **[test_oppo_sidekey_v0.4.4.zip](https://github.com/zhujiu39/findx8s-sidekey/releases/download/v0.4.4/test_oppo_sidekey_v0.4.4.zip)**，在 KernelSU 管理器中安装并重启。Release 附带 `SHA256SUMS.txt`；GitHub 自动生成的 **Source code** 压缩包用于开发，不能直接安装。

> [!IMPORTANT]
> 当前是 **v0.4.4 功能测试版**。侧键输入设备与键码已通过 ADB 确认，本地构建和自动化测试已通过；快捷菜单在 ColorOS 上的弹出与点击，以及系统手电筒、图标同步、震动和锁屏／息屏响应仍需目标手机验证。详细范围见[兼容性与验证状态](#compatibility)。
...
| 按名称选择应用 | 读取当前手机的桌面应用列表，支持搜索、刷新；普通启动、小窗启动和菜单卡片共用选择器 |
...
1. 从 [Releases](https://github.com/zhujiu39/findx8s-sidekey/releases/tag/v0.4.4) 下载 `test_oppo_sidekey_v0.4.4.zip`。
...
2. 在 **“你的快捷菜单”** 中点击 **“＋ 添加应用卡片”**，点击 **“选择应用”**，按名称搜索并选中手机中的应用；卡片名称会自动填写，也可以自行修改并设置 emoji。快捷开关通过 **“＋ 添加快捷开关”** 配置动作。上方固定 **2 行 × 4 列**，下方固定 **4 个开关位置**；未配置项保留占位且不能执行。
...
菜单打开时，同一层半透明背景覆盖整个窗口，包括状态栏后方；安全边距只用于菜单面板定位。v0.4.4 修正了旧版容器边距裁剪遮罩、留下顶部亮带的问题，并关闭系统栏额外添加的对比度底色。系统栏图标仍由系统绘制，不强制隐藏状态栏。
...
**v0.4.2 针对 `cmd: Failure calling service activity: Failed transaction (2147483646)` 修正了启动和安装路径。** 旧版会把模块私有日志文件描述符传入 Android 系统服务；新版改用管道传递标准输入输出，由模块保存日志，避免这条路径上的文件访问问题。启动成功还须同时满足 `am` 命令完成和菜单组件握手，最长等待 8 秒；失败不会再同时报告“动作退出码 0”。这项修复已经通过本地编译与回归测试，仍需目标手机复测，不代表已排除其他 ColorOS 启动限制。

### 按名称选择手机应用

在手势或菜单中选择 **“启动应用”** 或 **“启动应用（请求小窗）”**，点击应用选择按钮即可。弹窗列出当前 Android 用户拥有桌面启动入口的应用；输入应用名称可搜索，包名仅作为区分同名应用的辅助信息，无需手动填写。

- 点击 **“刷新”** 重新读取；应用列表最多在当前 WebUI 内存中缓存 10 秒，不写入文件或上传。
- 选择菜单应用时自动填写名称；更换应用会更新默认名称，手动起的名称会保留。仍需点击页面底部 **“保存设置”** 才会应用。
- 无桌面入口的系统服务不会出现在列表中。当前仅查询当前 Android 用户，不合并其他用户或工作资料的应用。
- 读取失败会显示原因并允许重试，保留已有配置。普通电脑浏览器无法获取手机应用，不会用示例列表冒充真实结果。

### 从旧版升级

- 在 KernelSU 中覆盖安装新版并重启，已有配置会保留；v0.4.4 不自动改动你的手势、菜单内容或位置设置。
...
每次完整构建都会执行 C、Java、JavaScript 测试，以及配置往返、Shell 语法、ELF、DEX、APK 签名和 ZIP 校验，并新建独立的 `交付文件/时间_test_v0.4.4_菜单顶部遮罩修复/` 目录。目录内包含安装包、源码与测试、说明文档、真实构建日志和 SHA256。
...
| 版本 | 主要变化 |
| --- | --- |
| **v0.4.4** | 修复顶部遮罩被安全边距裁剪形成的亮带；安全边距只约束面板，系统栏保持透明并使用统一背景遮罩 |
