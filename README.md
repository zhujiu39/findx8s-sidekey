...
4. 通过 **“＋ 添加快捷开关”** 配置顶部的两个开关。开关可以设置手电筒或其他动作；原来的额外非应用捷径会保留在 WebUI，移动到前两项即可显示，不会在升级时静默删除。
5. 点击 **“保存设置”** 后生效。用手势卡片中的 **“测试”** 打开手机菜单；**“预览滑出效果”** 仅展示外观。

侧栏按新的参考图重做：**顶部两个纵向圆角开关，下方应用固定两列**，真实应用图标下方显示名称。去掉大标题和逐格卡片边框，宽度上限 **196 dp**。内容超出窗口时上下滑动，横屏仍保持双列，避让状态栏、挖孔和导航区域。默认从左侧滑出，中心高度为 35%，可在 WebUI 调整方向和高度。深灰／浅灰配色跟随系统深色模式。

**手电筒**的开关状态来自系统回调：绿色表示开启、灰色表示关闭、横线表示暂不可读。其他单次动作显示执行符号，不伪造持续开／关状态。未配置的顶部位置显示不可点击占位。
