...
“手感调节”新增 **“触发时震动”**，默认开启，升级旧配置也默认开启。关闭后点击“保存设置”即可停用反馈，动作设置会保留。

有效短按／双击／长按被识别并加入动作队列时，请求一次 35 ms 短震动。长按在达到阈值时震动一次，继续按住或松手不重复震动；设为“不执行动作”、接管暂停或动作队列已满时不触发反馈。WebUI 的动作测试也会包含这次震动。

震动表示手势已接收，不表示后续动作必定成功。调用系统 vibrator_manager 服务，遵循系统震动和勿扰策略；反馈请求在独立子进程执行，最多 2 秒超时，失败只记录日志，不阻止动作。没有等待震动完成后再开灯的业务阻塞。
